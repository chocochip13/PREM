P_values = read.csv("prem_entries_value.csv")
P_labels = read.csv("prem_entries_lables.csv")


#Checking for One to One relation between labels and entries
as.factor(P_values$X_id == P_labels$X_id)
...
### Cross check
P_labels_v2 = subset(P_values,check_labels_v2==TRUE )
as.factor(P_values_v2$X_id == P_labels_v2$X_id)
...
###Cross Check
P_labels_v3_v4 = subset(P_labels, check_labels_v3_v4 ==TRUE)
as.factor(P_values_v3_v4$X_id == P_labels_v3_v4$X_id)
...
a[]
###Cross Check
P_labels_v4 = subset(P_labels_v3_v4, check_label_v4 == TRUE)
as.factor(P_values_v4$X_id ==  P_labels_v4$X_id)

#Extracting Version 3
P_values_v3 = subset(P_values_v3_v4, check_label_v4 == FALSE)
###Cross Check
P_labels_v3 = subset(P_labels_v3_v4, check_label_v4 == FALSE)
as.factor(P_values_v3$X_id ==  P_labels_v3$X_id)
...
#Analysing data
P_entries_v1 = read.csv("Processed data + IITM data/PREM_entries_v1.csv")
P_entries_v2 = read.csv("Processed data + IITM data/PREM_entries_v2.csv")
P_entries_v3 = read.csv("Processed data + IITM data/PREM_entries_v3.csv")
P_entries_v4 = read.csv("Processed data + IITM data/PREM_entries_v4.csv")

#Checking the difference in Outcome Variables
table(P_entries_v1$Outcome)
table(P_entries_v2$Outcome)
table(P_entries_v3$Outcome)
table(P_entries_v4$Outcome)

##Version 2 and Version 4 are the legit ones
##Version 2's Outcome has three levels and Version 4's Outcome has four levels
...
write.csv(v2_iitm2, file = "Totalli Processed/PREM_entries_v2.csv")
write.csv(v4_iitm2, file = "Totalli processed/PREM_entries_v4.csv")
## Writing the data in a folder

#Substituting NA for the missing values
P_v2 = read.csv("Totalli Processed/PREM_entries_v2.csv", header=T, na.strings=c("","[]","NA"))
P_v4 = read.csv("Totalli processed/PREM_entries_v4.csv", header=T, na.strings=c("","[]","NA"))

write.csv(P_v2 , file = "Totally Processed/PREM_entries_v2.csv" , row.names = FALSE)
write.csv(P_v4 , file = "Totally processed/PREM_entries_v4.csv" , row.names = FALSE)

P_v2 = read.csv("Totally Processed/PREM_entries_v2.csv")
P_v4 = read.csv("Data/Totally processed/PREM_entries_v4.csv")

############################ Missing Values for Version 2 ############################
#Finding the Missing ABCD values
P_v2_ABCD_na = subset(P_v2 , is.na(P_v2$Airway) == "TRUE" | is.na(P_v2$Breathing) == "TRUE" | is.na(P_v2$Circulation) == "TRUE" | is.na(P_v2$Disability) == "TRUE")

## writing the Missing ABCD
write.csv(P_v2_ABCD_na , file = "Missing Entries Data/PREM_entries_v2_missingABCD.csv" , row.names = FALSE)

#Finding the Missing Etiology Values
P_v2_etiology_na = subset(P_v2, is.na(P_v2$Etiology) == "TRUE")
## writing the Missing Etiology
write.csv(P_v2_etiology_na , file = "Missing Entries Data/PREM_entries_v2_missingEtiology.csv" , row.names = FALSE)

#Finding Missing Outcomes
P_v2_outcome_na = subset(P_v2, is.na(P_v2$Outcome) == "TRUE" )
## writing the Missing Outcome
write.csv(P_v2_outcome_na , file = "Missing Entries Data/PREM_entries_v2_missingOutcome.csv" , row.names = FALSE)
...
############################ Missing Values for Version 4 ############################
#Finding the Missing ABCD values
P_v4_ABCD_na = subset(P_v4 , is.na(P_v4$Airway) == "TRUE" | is.na(P_v4$Breathing) == "TRUE" | is.na(P_v4$Circulation) == "TRUE" | is.na(P_v4$Disability) == "TRUE")

## writing the Missing ABCD
write.csv(P_v4_ABCD_na , file = "Missing Entries Data/PREM_entries_v4_missingABCD.csv" , row.names = FALSE)

#Finding the Missing Etiology Values
P_v4_etiology_na = subset(P_v4, is.na(P_v4$Etiology) == "TRUE")
## writing the Missing Etiology
write.csv(P_v4_etiology_na , file = "Missing Entries Data/PREM_entries_v4_missingEtiology.csv" , row.names = FALSE)

#Finding Missing Outcomes
P_v4_outcome_na = subset(P_v4, is.na(P_v4$Outcome) == "TRUE" )
## writing the Missing Outcome
write.csv(P_v4_outcome_na , file = "Missing Entries Data/PREM_entries_v4_missingOutcome.csv" , row.names = FALSE)


#Data with only valid data
P_total_data_v2 = read.csv("Totally processed/PREM_entries_v2.csv") 
P_total_data_v2$X.1 <- NULL
P_total_data_v2$X <- NULL
ABCD = P_total_data_v2
#######################################Codes Replacement################################################################
...
ABCD$Breathing = gsub(1,"Apnea", ABCD$Breathing)
...
P_total_data_v2 = ABCD

P_valid_data_v2 = subset(P_total_data_v2, !is.na(P_total_data_v2$Airway)) 
P_valid_data_v2 = subset(P_valid_data_v2, !is.na(P_valid_data_v2$Breathing)) 
P_valid_data_v2 = subset(P_valid_data_v2, !is.na(P_valid_data_v2$Circulation))
P_valid_data_v2 = subset(P_valid_data_v2, !is.na(P_valid_data_v2$Disability))
P_valid_data_v2 = subset(P_valid_data_v2, !is.na(P_valid_data_v2$Etiology))
P_valid_data_v2 = subset(P_valid_data_v2, !is.na(P_valid_data_v2$Outcome))
write.csv(P_valid_data_v2 , file = "Valid data/P_valid_data_v2.csv", row.names = FALSE)

#Data with only valid data
P_total_data_v4 = read.csv("Totally processed/PREM_entries_v4.csv") 
P_total_data_v4$X.1 <- NULL
P_total_data_v4$X <- NULL

ABCD = P_total_data_v4 
#######################################Codes Replacement################################################################
#Replacing Codes for Airway
ABCD$Airway = gsub(0,"Clear airway" , ABCD$Airway)
ABCD$Airway = gsub(1,"Obstructed" , ABCD$Airway)
...
P_total_data_v4 = ABCD

P_valid_data_v4 = subset(P_total_data_v4, !is.na(P_total_data_v4$Airway)) 
P_valid_data_v4 = subset(P_valid_data_v4, !is.na(P_valid_data_v4$Breathing)) 
P_valid_data_v4 = subset(P_valid_data_v4, !is.na(P_valid_data_v4$Circulation) )
P_valid_data_v4 = subset(P_valid_data_v4, !is.na(P_valid_data_v4$Disability))
P_valid_data_v4 = subset(P_valid_data_v4, !is.na(P_valid_data_v4$Etiology))
P_valid_data_v4 = subset(P_valid_data_v4, !is.na(P_valid_data_v4$Outcome))
write.csv(P_valid_data_v4 , file = "Valid data/P_valid_data_v4.csv", row.names = FALSE)
